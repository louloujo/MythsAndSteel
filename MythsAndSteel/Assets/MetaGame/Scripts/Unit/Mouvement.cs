...
public class Mouvement : MonoSingleton<Mouvement>
{
    ...
    private void DisplayMoveArrow()
    {
        if (mUnit.GetComponent<UnitScript>().MoveLeft != 0)
        {
            ...
            {
                TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] + 9].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, UpArrow);
            }

            if (newNeighbourId.Contains(selectedTileId[selectedTileId.Count - 1] - 9))
            {
                TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] - 9].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, DownArrow);
            }

            if (newNeighbourId.Contains(selectedTileId[selectedTileId.Count - 1] + 1))
            {
                TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] + 1].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, RightArrow);
            }

            if (newNeighbourId.Contains(selectedTileId[selectedTileId.Count - 1] - 1))
            {
                TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] - 1].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, LeftArrow);
            }
        }

        if (selectedTileId.Count > 2)
        {
            if (selectedTileId.Count - 2 == 9 || selectedTileId.Count - 2 == -9)
            {
                if (selectedTileId.Count - 3 == 10)
                {
                ...
                else if (selectedTileId.Count - 3 == 8)
                {
                ...
                else if (selectedTileId.Count - 3 == -10)
                {
                    TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] + 9].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, Virage3);
                }
                else if (selectedTileId.Count - 3 == -8)
                {
                    TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] + 9].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, Virage3);
        ...
        if (selectedTileId.Count > 2)
        {
            if (selectedTileId.Count - 2 == 1 || selectedTileId.Count - 2 == -1)
            {
                if (selectedTileId.Count - 3 == 10)
                {
                    TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 2]].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, Virage1);
                ...
                {
                    TilesManager.Instance.TileList[selectedTileId[selectedTileId.Count - 1] + 9].GetComponent<TileScript>().ActiveChildObj(MYthsAndSteel_Enum.ChildTileType.MoveSelect, Virage4);
                }
                else
                {
